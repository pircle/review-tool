...
@click.pass_context
def init(ctx, path, name, languages, exclude_dirs, exclude_files, include_dirs, standards, non_interactive):
    ...
    try:
        ...
        if existing_project:
            if not non_interactive and not click.confirm(f"Project '{project_name}' already exists. Do you want to update its configuration?"):
        ...
        if not non_interactive:
            ...
            sys.stdout.flush()  # Flush after prompt
        ...
        try:
            ...
            click.echo(f"Failed to create project directories: {str(e)}", err=True)
            ...
            if config_manager.set_current_project(project_name):
                ...
                try:
                    if config_manager.save_project_config():
                        ...
                        sys.stdout.flush()  # Flush success message
                    else:
                        click.echo("Failed to update project configuration", err=True)
                        sys.stdout.flush()  # Flush error message
                        return
                except OSError as e:
                    logger.error(f"Failed to save project configuration: {str(e)}")
                    click.echo(f"Failed to save project configuration: {str(e)}", err=True)
                    sys.stdout.flush()  # Flush error message
                    return
            else:
                logger.error(f"Failed to set current project to '{project_name}'")
                click.echo(f"Failed to set current project to '{project_name}'", err=True)
                sys.stdout.flush()  # Flush error message
                return
        else:
            try:
                if config_manager.add_project(project_name, abs_path):
                    if config_manager.set_current_project(project_name):
                        ...
                        if config_manager.save_project_config():
                            ...
                            click.echo("Failed to save project configuration", err=True)
                            sys.stdout.flush()  # Flush error message
                            return
                    else:
                        logger.error(f"Failed to set current project to '{project_name}'")
                ...
                click.echo(f"Failed to create/update project: {str(e)}", err=True)
                sys.stdout.flush()  # Flush error message
                return
        
        # Log the initialization
        logger.info(f"Project '{project_name}' initialized at {abs_path}")
        sys.stdout.flush()  # Final flush before returning
        
    except Exception as e:
        logger.error(f"Error during project initialization: {str(e)}")
        if logger.getEffectiveLevel() == logging.DEBUG:
            logger.error(traceback.format_exc())
        click.echo(f"Error during project initialization: {str(e)}", err=True)
        sys.stdout.flush()  # Ensure error output is flushed
        return
